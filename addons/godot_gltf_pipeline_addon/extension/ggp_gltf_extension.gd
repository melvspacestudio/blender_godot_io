@tool
class_name GGP_GLTF_Extension extends GLTFDocumentExtension

static var TAG:
	get: return "GGP_GLTF_Extension"
	
func _import_preflight(state: GLTFState, extensions: PackedStringArray) -> Error:
	var list := GGP_ImportConfig.get_config(state.base_path)
...
func _import_node(state: GLTFState, gltf_node: GLTFNode, json: Dictionary, node: Node) -> Error:
	...
	for config in list.configs:
		for modifier in config.modifiers:
			...
			if node != new_node:
				GGP_NodeUtility.replace(node, new_node)
				node = new_node
