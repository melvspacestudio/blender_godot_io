...
func _apply_later(node: Node, properties: Dictionary[String, Variant]):
	...
	if not apply_later_node or apply_later_node is not BGIO_ImporterMeshInstance3D_ApplyLater:
		apply_later_node = BGIO_ImporterMeshInstance3D_ApplyLater.new()
		node.add_child(apply_later_node)
		
		apply_later_node.owner = node.owner
		apply_later_node.name = "_ApplyLater"
	
	apply_later_node = apply_later_node as BGIO_ImporterMeshInstance3D_ApplyLater
